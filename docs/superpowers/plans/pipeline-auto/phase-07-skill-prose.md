...
The terminal report's "attested-only completions" list reads the `range:<attempt>@<proof_mode>` checkpoint marker. **No public spelling is owed for it, and this paragraph used to claim one.** The reader of that marker is `render_terminal_report`, which is P06 module code appended to `pipeline_auto_state.py` — it reaches `_RANGE_CHECKPOINT` directly, and a private call from module code is not a phase-boundary crossing. What this stage's prose does is *describe a grammar to an agent that has no module*, and prose necessarily spells a grammar literally: the paragraph above already spells `quorum:<qid>@<path>#sha256=<digest>` in full for the same reason. The related claim that this debt "blocks P06's fix-round writer" is also withdrawn — measured, `_resolve_question_record` could not have served that call site whether public or private, and P06 now writes the **halt** arm there, which is the arm for a block that opened no quorum.
