...
| Action | Grants |
| --- | --- |
| `task.resume` | One blocked task's `[?] → [~]` for a named attempt. It binds to the block through the task's `Question` cell: `quorum:<qid>@<path>#sha256=<digest>` (the decision must be `Q-<that qid>`) or `halt:<reason>` (human only, with the reason repeated in `Blocker`) |
| `quorum.adopt` | One quorum-adopted answer for one qid |
