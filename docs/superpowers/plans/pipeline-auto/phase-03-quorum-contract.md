...
11. **Never ask `is_file()` (or `exists()`) to mean "there is nothing here".** `is_file()` is false for a DIRECTORY, a DANGLING symlink, a symlink LOOP and a FIFO — every one of which is a name the directory *carries* and cannot be read. Folding them into "absent" is the fail-open direction: the run proceeds as though nothing was decided, nothing was answered, nothing was recorded. **Ask existence of the NAME and readability of the TARGET**: `os.path.lexists` is satisfied by a dangling link and by a loop, which `exists()` cannot distinguish, and `read_text` must be reached only for a regular file — which is also what stops a FIFO BLOCKING a call made under the run lock until a writer that never comes. This has now been found twice in consecutive tasks, both times by mutation and never by a test anyone wrote: `_decisions_text` read an unreadable audit trail as an undecided run, and a response file that existed and could not be read counted as a brain that had not answered.
