...
On Claude Code this ships as a real agent at `agents/bug-investigator.md` and is
invoked as `superb:bug-investigator`. Where no bundled agent is available — Codex
has no `agents` manifest key — the skill dispatches a generic subagent carrying
this file instead. The two must stay identical between the SHARED BRIEF markers;
