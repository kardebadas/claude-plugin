...
8. Formal reviewer reports are stored in `agent-output/`; findings and dispositions live in `findings.md`; gate/round state and evidence references live in `progress.md`. Gate acceptance is computed from matching reports/code state, digest-bound recorded verification, blockers/questions, Minor dispositions, strict post-review fix provenance, and re-review evidence—not accepted from a boolean flag.
