...
<!-- pipeline-v2-task: id=P2-T01; deps=none; batch=P2-pressure; order=1; write_scope=docs/superpowers/runs/2026-09-08-pipeline-rebuild-v2/agent-output -->
...
- [ ] **Step 3:** Run at least five fresh no-guidance micro-test contexts for the same unresolved-choice wording; save p2-micro-no-guidance-01 through p2-micro-no-guidance-05 (or higher), and manually score the action and rationalization in every record.
- [ ] **Step 4:** Preserve exact rationalizations, not summaries; these are the only failures later wording addresses.
- [ ] **Step 5:** Record the reusable evidence index mapping all RED/micro-control files to their scenario and intended Phase 4 acceptance use; runtime evidence remains ignored and uncommitted.
- [ ] **Step 6:** Verify RED evidence and source cleanliness.
...
Expected: six full-pressure RED records, at least five no-guidance micro-controls, and no Pipeline source diff.

### Task P2-T02: Rewrite the compact control-plane router
<!-- pipeline-v2-task: id=P2-T02; deps=P2-T01; batch=P2-control-plane; order=1; write_scope=plugins/superb/skills/pipeline/SKILL.md -->
...
! rg -n 'references/(run-state|implement|parallel|fix-loop)\.md|Brain Agent|ceil\(|RVJ|recursive.*fix-mode|finishing-a-development-branch' plugins/superb/skills/pipeline/SKILL.md
~~~
...
<!-- pipeline-v2-task: id=P2-T03; deps=P2-T01; batch=P2-planning; order=1; write_scope=plugins/superb/skills/pipeline/references/planning.md -->
...
**Acceptance behavior:** Require superpowers:brainstorming before user-approved design and superpowers:writing-plans for master/phase plans. Only explicit user instructions, recorded answers, approved artifacts, and repo rules resolve choices; otherwise persist/ask and block affected work. Require an explicit positive capacity-compatible worker_limit, at-most-12 genuine tasks, the stated strict metadata with exact paths/commands, behavior-focused coverage policy, and approval of master plus every phase plan before implementation. Phase 4 owns A-01–A-20 evidence; runtime-state survival limits are disclosed.

- [ ] **Step 1:** Translate P2-T01 unresolved-choice/capacity/remote controls into source-citation-or-stop rules.
- [ ] **Step 2:** Create planning.md with exact metadata fields Stable ID, Depends on, Batch, Write scope, and Verification.
...
! rg -n 'Brain Agent|lane:|RVJ|ceil\(' plugins/superb/skills/pipeline/references/planning.md
~~~
...
<!-- pipeline-v2-task: id=P2-T04; deps=P2-T01; batch=P2-execution; order=1; write_scope=plugins/superb/skills/pipeline/references/execution.md -->
...
**Acceptance behavior:** Require superpowers:test-driven-development for testable behavior and superpowers:systematic-debugging for unexpected failures. Before dispatch the controller validates dependencies, decisions, limit/capacity, batch order, and disjoint scopes, then persists [~], owner, attempt. Workers cannot edit tracker/spawn helpers; they publish atomic attempt results with task checkpoints. Controller validates plan/result/Git/checks; failure of a planned integrated suite is implementation repair, never formal review. Every phase has fresh planned-suite evidence on integrated HEAD before final-only/required classification.
...
<!-- pipeline-v2-task: id=P2-T05; deps=P2-T01; batch=P2-persistence; order=1; write_scope=plugins/superb/skills/pipeline/references/persistence.md,plugins/superb/skills/pipeline/templates/decisions.md -->
...
**Acceptance behavior:** All commands validate before write. Controller mutation is lock → read → validate → transition → validate → same-directory temporary write/flush/fsync → atomic replace → directory sync where supported → unlock; no unlocked fallback, stale-lock deletion, truncation, or reinitialization. Status is read-only. Resume is file-first reconciliation of plans, decisions, findings/fix plan, Git/worktree evidence, and every in-progress attempt. V1 recognition uses actual v1 grammar and emits unchanged/no-dispatch diagnostics. Consume and verify Phase 1's progress template (v2 marker/identity/paths/current phase-batch-next action, task/phase/gate/remediation records) and worker-result template (run/task/attempt/status/checkpoints/commits/tests/evidence/concerns/questions plus atomic publication); do not modify either. Create decisions.md with stable question/answer/source/status. State D-009's one-host local-filesystem scope and native-versus-simulated Windows limitation without claiming power-loss durability.
...
<!-- pipeline-v2-task: id=P2-T06; deps=P2-T01; batch=P2-review; order=1; write_scope=plugins/superb/skills/pipeline/references/review.md,plugins/superb/skills/pipeline/templates/findings.md,plugins/superb/skills/pipeline/templates/fix-plan.md -->
...
**Acceptance behavior:** final-only phases get no formal phase reviewer. A required gate has a consequence-based reason and exactly one independent reviewer only after full integration/mechanical verification. The mandatory master gate has exactly two independent reviewers on the same recorded base/HEAD: A covers requirements/behavior/decisions; B covers integration/reliability/recovery/concurrency/security/test quality. Both finish before consolidation; no third reviewer or task reviewer. Verified Critical/Important block, approved missing requirements cannot be Minor, all Minors are Fixed/Deferred-with-authority-impact/Rejected-with-evidence. Persist immutable gate/round/scope/targets/fix-plan/commits/verification/reports/outcome before dispatch. Initial review is round zero; three shared non-recursive fix/re-review rounds maximum, same identity on resume, and D-006 early stops/escalation apply.
...
! rg -n 'Major|RVJ|ceil\(|recursive.*fix-mode|per-task review|per-finding' plugins/superb/skills/pipeline/references/review.md plugins/superb/skills/pipeline/templates/findings.md plugins/superb/skills/pipeline/templates/fix-plan.md
~~~
...
<!-- pipeline-v2-task: id=P2-T08; deps=P2-T02,P2-T03,P2-T04,P2-T05,P2-T06,P2-T07; batch=P2-pressure; order=2; write_scope=docs/superpowers/runs/2026-09-08-pipeline-rebuild-v2/agent-output -->
...
**Acceptance behavior:** Re-run each identical full-pressure RED prompt fresh with v2 router and only necessary stage references. Separately run the same unresolved-choice wording with the same v2 guidance variant at least five times in fresh contexts; manually score each v2-guidance micro-test against each no-guidance control. GREEN passes only where the agent cites an explicit source or returns allowed stop status; preserves incompatible state; queues/blocks conflicting work despite capacity; reconciles rather than accepts/duplicates contradictory post-commit evidence; applies correct review timing/composition; and refuses remote actions. A new rationalization triggers a narrow fix in its owning task and repeat of that scenario. The paired RED/GREEN and 5+5 micro-test records are reusable Phase 4 evidence and must not be sampled again there; actual-agent and deterministic simulation remain separately labelled.

- [ ] **Step 1:** Run and atomically save six p2-green-<scenario>.md records with the same full-pressure prompts as RED.
- [ ] **Step 2:** Run and save at least five p2-micro-v2-guidance-<n>.md records using the exact unresolved-choice wording and the same v2-guidance variant; manually score all 5+5 micro-test records rather than relying on counts.
- [ ] **Step 3:** Manually compare each full-pressure pair and each micro-test set, recording control result, GREEN behavior, source citation, new rationalization, and its reusable Phase 4 acceptance mapping.
...
! rg -n 'references/(run-state|implement|parallel|fix-loop)\.md|templates/(register|implementer-prompt|kit)\.md|Brain Agent|ceil\(|RVJ|recursive.*fix-mode|finishing-a-development-branch|external /review' plugins/superb/skills/pipeline
...
Expected: helper suite/structural checks pass, six GREEN records exist, and no retired active route remains.
...
- [ ] **Step 6:** Hand off fresh integrated evidence only. Runtime pressure evidence remains ignored and uncommitted. Failures return to owning task under TDD/systematic-debugging, never formal remediation.

## Phase verification and review gate

**Mechanical verification:** On integrated Phase 2 HEAD: P2-T08 paired RED/GREEN real-agent evidence; Phase 1 helper suite; compact-router/reference checks; stale-v1/SDD/per-task-review scans; helper/template contract checks; two named no-op-aware simulations; and git diff --check. Report unavailable platform/runtime checks rather than claiming them.
