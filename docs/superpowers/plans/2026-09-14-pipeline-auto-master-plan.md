...
- **`is_file()` never means "there is nothing here".** It is false for a directory, a dangling symlink, a symlink loop and a FIFO — all of which exist and cannot be read. Folding them into "absent" is fail-open. Ask existence of the name (`os.path.lexists`) and readability of the target (`read_text` only for a regular file, which also stops a FIFO blocking under a lock). Found twice in consecutive tasks, both times only by mutation. **And the door that asks the shape RAISES as well as answering.** `Path.is_file()` swallows only `pathlib._IGNORED_ERRNOS` — measured on CPython 3.11 as `(ENOENT, ENOTDIR, EBADF, ELOOP)` — and re-raises every other `OSError`. P04 Task 5 added `_require_regular_file` to `resolve_evidence` under an `except QuorumError` and so introduced a NEW escape from `TrackerError` in the commit that closed the FIFO one: a 312-character evidence reference left the family as `OSError [Errno 36] File name too long` and a search root at mode `000` as a raw `PermissionError`, both of which had been in-family one commit earlier, and the first is a pure argument that the spelling screen accepts. **Wrap the door where the door is defined, not at the call site that found the hole**: seven call sites share it — `_question_record`, `payload_digest`, `_read_json`, `_response_record`, `_plan_text`, `resolve_evidence` and `_ref_text`, enumerated from the module's AST rather than remembered — and a local `except OSError` at `resolve_evidence`, the one that found the hole, would have been a rule scoped to where it was learned while the other six stayed open. **Re-enumerate the count before quoting it.** It has moved twice since this bullet was first written — five, then six, now seven, `_ref_text` arriving with P04 Task 6 and inheriting the wrap without knowing it existed — and a number that drifts silently is one the next reader trusts wrongly, which is this plan's own "name the artefact that produces the number" rule turned on the plan.
...
- **A quantitative claim in a report is a claim to CHECK: name the artefact that produces the number, or do not state the number.** P04 Task 5's brief and report both described a **"60,000-document fuzz"**. No such corpus existed — not in the suite, not in any report, not anywhere in the repository; the real artefact was a 972-document both-directions cartesian sweep. The figure nonetheless passed a review that stated it had checked the task, because a number reads as evidence and a reviewer confirming a report tends to confirm its arithmetic rather than its referent. A figure with no referent is worse than no figure: it retires the question it was invented to answer. **Every count, size, percentage or timing in a report must name the file that produces it and the command that runs it**, and a reviewer checks the referent before the value. The fuzz itself now exists — `scratchpad/p04t05/fuzz_envelope.py`, deterministic under a fixed seed, 60,000 perturbed documents, 0 accepted under a second sha256 and 0 escapes from `TrackerError` — which is what the claim should have pointed at in the first place.
