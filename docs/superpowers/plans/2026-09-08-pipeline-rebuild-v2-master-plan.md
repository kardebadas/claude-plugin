...
- Runtime decisions D-001 through D-022 in `docs/superpowers/runs/2026-09-08-pipeline-rebuild-v2/decisions.md` are binding. D-017 is a one-time pre-release adoption and D-019 is a one-time historical outcome reconciliation for this exact rebuild run; neither is an ordinary future resume/migration path. D-021 authorizes only Round 4 and D-022 only Round 5 of the existing `phase-01` gate; both leave D-006's three-round default unchanged elsewhere and neither authorizes the following round.
...
- Critical/Important findings block. Every Minor has a recorded disposition. Each formal gate allows at most three ordinary remediation rounds under D-006; any finite extension requires its own explicit persisted exact-run authority. D-021 supplies only this rebuild's `phase-01` Round 4 authority; D-022 separately supplies only Round 5 and does not authorize Round 6.
