...
| `plugins/superb/skills/pipeline/templates/verification-evidence.md` | Digest-referenced PASS evidence with exact tested code-state identity | Create |
...
Phase 1 owns only the three state/evidence-contract templates above. Phase 2 owns the prose control plane and the decisions/findings/fix-plan templates, and must consume—not redefine—the interfaces fixed here. No Phase 1 task edits `SKILL.md`, current v1 references, shared validators, CI, manifests, setup, or README.
...
**Files:** Create the helper/test/fixture paths from the map; rewrite `templates/progress.md`; create `templates/worker-result.md` and `templates/verification-evidence.md`.
...
@contextmanager
def bind_runtime_capacity_provider(run_dir: Path,
                                   provider: RuntimeCapacityProvider) -> Iterator[None]: ...
...
def advance_phase(run_dir: Path, *, completed_phase_id: str,
                  next_phase_id: str) -> Tracker: ...
```

`start_task` is first-start only. `resume_task` validates that the task is `[?]`, the blocked attempt matches, the new attempt is distinct and unused, and `decision_ref` resolves in the recorded decisions artifact to an explicit resolved answer applicable to this task/current blocker. Missing, pending, unrelated, duplicated/conflicting, or generic plan-approval evidence fails unchanged. Under D-018, the controller binds a provider for the canonical run; the shared narrow start guard queries it inside the tracker lock and checks current dependency integration/completion, the persisted limit plus fresh runtime capacity, ownership, and typed-scope conflicts before either transition. Missing/unknown/failed/insufficient capacity fails unchanged, and recovery must rebind/re-detect before a new start. P1-05 later adds candidate calculation and multi-task reservation, not a second start policy or capacity authority. Resume appends stable prior-attempt/result/question/checkpoint references before replacing the current owner/attempt, and the identical already-applied resume is idempotent without consuming another capacity observation. Session recovery never calls `resume_task` merely because context was lost.

One shared internal derivation refreshes the persisted `next_action` summary inside the same locked update after task start/block/resume/import, integration, phase verification, review/remediation changes, and phase advancement. It derives from underlying approved plan/tracker/evidence facts and never changes those facts, advances a phase, dispatches, or resets history. `advance_phase` validates the current completed phase, all task-kind/integration evidence, phase verification, required gate acceptance, absence of unresolved questions, and the approved immediate successor/dependencies before atomically changing the phase cursor and deriving that phase's first permitted action. Exact replay is a no-op; stale/skipped/conflicting requests fail unchanged. A verified final phase derives the mandatory master gate rather than DONE.

**Applicable decisions:** D-004, D-008, D-011, D-013, D-018, D-023.
...
The derived-action and phase-cursor contract is part of this task: completing the final task selects outstanding integration or verification rather than the completed task; an unresolved required phase review prevents advancement; an accepted required phase and a verified final-only phase can advance only to their approved immediate successor; wrong-phase, skipped-dependency, and unresolved-question requests fail byte-for-byte unchanged; exact advancement replay is idempotent; and the last verified phase routes to the master gate rather than `DONE`. Read-only inspection may report a stale persisted summary and the derived value but does not rewrite the tracker.

- [ ] **RED — add legal/illegal transition tests.** Assert the exact row before/after `[ ] → [~] → [x]` and `[~] → [?] → [~]`, plus `test_start_task_accepts_initial_and_rejects_blocked_active_completed`, `test_resume_task_accepts_matching_blocked_attempt_and_applicable_resolved_decision`, `test_resume_rejects_missing_unresolved_unrelated_or_conflicting_decision_unchanged`, `test_resume_rejects_reused_stale_attempt_and_completed_task`, `test_identical_resume_replay_has_no_additional_effect`, `test_late_prior_attempt_result_cannot_complete_resumed_task`, `test_context_recovery_does_not_create_new_attempt`, `test_direct_start_and_resume_require_fresh_bound_runtime_capacity`, `test_stale_readiness_cannot_authorize_direct_start`, `test_capacity_provider_failure_leaves_tracker_bytes_unchanged`, `test_same_owner_batch_counts_one_worker`, `test_valid_artifact_task_completes_without_source_commit`, `test_artifact_task_missing_expected_evidence_is_rejected`, `test_source_task_without_implementation_commit_is_rejected`, `test_result_commit_need_not_reach_target_before_integration`, `test_proper_worker_commit_ancestry_allows_integration`, `test_unrelated_target_commit_does_not_integrate_worker_commit`, `test_missing_or_conflicting_integration_evidence_blocks_dependents`, `test_illegal_transition_leaves_bytes_unchanged`, `test_stale_attempt_cannot_regress_or_increment`, and `test_start_persists_attempt_before_dispatch_callback`.

- [ ] **RED — reproduce stale derived action and missing phase advancement.** Complete the final task and prove the desired action is integration or phase verification, not the completed task. Add byte-preservation cases for an unaccepted required review, wrong current phase, skipped phase dependency, and unresolved question; prove accepted required and verified final-only predecessors advance to their immediate approved successor, replay is inert, the final phase routes to the master gate, and read-only inspection never rewrites stale tracker bytes.
...
- [ ] **GREEN — add D-011/D-013 lifecycle transitions, kind-aware completion, and complete integration proof inside the lock.** Resolve task metadata from the approved phase plan. Share one narrow start-guard/update core, while keeping `start_task` first-start only and `resume_task` answered-block only. Resolve the tracker decisions path, parse the referenced stable decision section, require one applicable explicit answer, exact `Decision action: task.resume`, and `Resolved` status, reject any other unresolved task-scoped decision, append prior attempt/evidence linkage, and persist the distinct new attempt before dispatch. Never infer controller authority from answer prose; missing, duplicated, unknown, wrong-purpose, or fieldless historical action fields grant no new action. For `source`, require verified checkpoint/result and at least one valid implementation commit reachable from its recorded source ref before `[x]`; for `artifact`, require the exact planned artifacts and validation evidence, set source fields `N/A`, and record integration `N/A`. Only `record_task_integration` may integrate a source task, and it must verify both `git merge-base --is-ancestor <each-implementation-commit> <integration_commit>` and `git merge-base --is-ancestor <integration_commit> <target_branch>`, then bind verification evidence to that integrated commit. Rewritten-history equivalence is unsupported unless later explicitly approved. Add one shared derived-action function and invoke it inside the same locked update for each relevant transition. Add `advance_phase` with evidence-derived acceptance, approved immediate-successor/dependency validation, exact replay handling, and final-phase routing to the existing master gate.
...
def start_remediation_round(run_dir: Path, *, gate_id: str, round_number: int,
                            ...
                            fixer_assignments: tuple[str, ...], capacity: int | None,
                            extension_decision_ref: str | None = None) -> Tracker: ...
...
**Applicable decisions:** D-004, D-005, D-006, D-007, D-008, D-014, D-015, D-017, D-018, D-019, D-020, D-021, D-022, D-023, D-024, D-025.

**Acceptance:** Failed/missing verification cannot open a gate; `final-only` cannot dispatch a phase reviewer; Phase 1 and Phase 2 each accept their different approved required reasons while missing/mismatched reasons fail; the master gate uses its own type and exactly two assignments. Reviewer and fixer reservations share the global worker ceiling and preserve released ownership. Task resume, review-question resolution, unknown-filesystem acknowledgement, and finite remediation extension each require exactly one matching closed-vocabulary `Decision action` plus their existing applicable scope/state/identity metadata; answer prose is never parsed as transition authority, and missing, duplicated, unknown, wrong-purpose, or fieldless historical actions fail unchanged. Closure reads only the tracker-authoritative findings path and is derived from reports/findings/evidence, not `accepted=True`, an empty list, or bare `Resolved`: new reports must identify the correct gate/base/HEAD/assignment and include D-029's JSON `outcomes` mapping; initial findings are `Open`, every active re-review assignment concludes every existing gate finding exactly once, and any open/missing/conflicting conclusion blocks. Already digest-sealed five-field reports remain readable only as historical lineage and cannot be submitted as new evidence. Verification must resolve to digest-bound PASS artifacts for that exact code state and equal the phase/round's recorded verification; confirmed Critical/Important and unresolved acceptance questions block; every resolved blocker has an evidence-backed Fixed/Rejected disposition; Rejected evidence resolves to an artifact with exact `Finding: <ID>` and one nonempty `Rationale:` field, never prefix/substring identity; every Minor has a valid disposition; and every repository-changing fix is bound to the exact targeted remediation round that actually resolved it, strict post-review commit, integrated fix HEAD, and matching re-review evidence. A historical round cannot prove Fixed for a finding listed in its own `remaining-blockers`. Per D-020, `gate.base` and `gate.reports` remain the immutable initial anchor while `gate.head` is the rolling accepted review cursor; evaluation validates the contiguous initial-plus-remediation evidence chain and never rewrites the initial report set. Remediation targets exactly the current open blockers, records exactly one outcome containing `none` or unique known blocker IDs for every completed round, and stops on no blocker progress or reintroduction/oscillation; empty, duplicate, unknown, and mixed-sentinel outcomes fail ordinary validation/resume. D-019 permits only the exact active rebuild controller to reconcile its one historical Round 1 omission from the recorded immutable re-review package under the normal lock, preserving Round 2 and every other state field; it is not available as automatic future-run repair or migration. Round zero costs no allowance; parallel fixes share one round; D-006 stop rules remain. D-021, D-022, D-023, and D-025 permit only this run's `phase-01` Rounds 4, 5, 6, and 7 respectively. Each extension validates exact run/source revision/gate state/question/predecessor/round/finding metadata, persists its authority on its row before fixes, preserves it through re-review, and cannot authorize the following round or alter other gates' default. Under D-024, remediation uses focused regressions before one committed integrated phase suite; reviewers validate applicable full-suite evidence and run focused adversarial probes without automatic duplicate full-suite execution.

- [ ] **RED — add positive/negative gate/remediation tests.** Include `test_mixed_source_and_artifact_phase_can_verify_without_fabricated_commit`, `test_phase_one_accepts_its_approved_reason`, `test_phase_two_accepts_its_different_approved_reason`, `test_missing_or_mismatched_required_reason_fails`, `test_final_only_phase_cannot_open_phase_review`, `test_master_gate_requires_two_matching_reviewers`, `test_new_reports_require_complete_explicit_finding_outcomes`, `test_conflicting_reviewer_outcomes_cannot_close_master_gate`, `test_caller_boolean_or_empty_findings_cannot_prove_acceptance`, `test_wrong_gate_or_reviewed_head_report_is_rejected`, `test_nonexistent_or_digest_mismatched_verification_is_rejected`, `test_gate_verification_must_match_recorded_phase_or_round_evidence`, `test_missing_verification_or_unresolved_question_blocks_closure`, `test_confirmed_blocker_blocks_closure`, `test_unchanged_review_head_cannot_be_a_fix`, `test_repository_fix_without_matching_rereview_blocks_closure`, `test_historical_round_cannot_fix_its_recorded_remaining_blocker`, `test_evidence_backed_rejection_can_close_a_gate`, `test_rejection_without_exact_finding_identity_fails`, `test_remaining_blocker_outcome_rejects_empty_duplicate_unknown_and_mixed_sentinel`, `test_missing_duplicate_unknown_or_wrong_decision_action_fails_unchanged`, `test_answer_prose_cannot_authorize_a_controller_transition`, `test_minor_target_cannot_manufacture_blocker_progress`, `test_reintroduced_blocker_stops_as_oscillation`, `test_unknown_filesystem_ack_requires_applicable_resolved_decision`, `test_initial_review_is_round_zero`, `test_parallel_fix_results_share_one_round`, `test_round_three_blocker_refuses_acceptance`, `test_round_four_requires_exact_finite_authority_and_preserves_history`, `test_round_five_authority_is_bound_to_exact_run_revision_and_predecessor`, and the D-019 exact-run tests proving evidence-backed outcome reconciliation, strict ordinary rejection, preserved Round 2 identity, byte-preserving conflicts, and idempotent replay.
...
- [ ] **GREEN — implement phase-specific gate and evidence-derived acceptance under the tracker lock.** Read classification/reason from the applicable approved phase metadata and compare the tracker to that phase; keep the literal Phase 1 reason only in its fixture. Reject a phase gate for `final-only`; separately enforce the master gate type and two D-007 assignments. Parse only the small documented reviewer-result header and verification-evidence record needed to bind report/gate/base/HEAD/assignment plus digest/PASS/code state. Derive closure from the authoritative recorded verification, required reports, validated findings/dispositions/questions, strict post-review fix commits, remaining-blocker history, and applicable re-review paths. Store paths as references; validate round/finding identity before dispatch changes.

- [ ] **GREEN check.**

  Run: `python3.11 -m unittest plugins.superb.skills.pipeline.tests.test_pipeline_state.PhaseGateAndRemediationTest -v`

  Expected: PASS; phase review is impossible before mechanical verification, post-limit rounds need exact finite authority, D-021 cannot authorize Round 5, and D-022 cannot authorize Round 6.
...
After the mechanical gate is green, dispatch exactly one independent reviewer over the integrated Phase 1 range. The reviewer checks strict state ownership, unsafe recovery/locking/atomic behavior, readiness/worker-limit correctness, test quality, the tracker/template contract, and the applicable recorded decisions. They may not review their own implementation. Critical or Important findings block, every Minor gets a persisted disposition, and remediation follows D-005/D-006 with at most three ordinary shared rounds. D-021, D-022, D-023, and D-025 are separate exact finite extensions for this rebuild's already-open gate; none changes future policy or authorizes another round. Under D-024, the reviewer validates the applicable committed full-suite evidence and runs focused adversarial probes; they rerun broader checks only when evidence is missing/inapplicable or a focused probe exposes broader risk, and record why. No later phase is eligible until this required gate is accepted.
