...
P1-01 through P1-04 use this bounded bootstrap. After P1-04's parser, atomic-write primitive, and task transitions pass their targeted tests, the controller creates an immutable bootstrap-evidence snapshot under `agent-output/`, renders one canonical v2 replacement for the same `progress.md`, validates it, acquires the new run lock, and atomically replaces that tracker in place. The canonical tracker seeds the already verified P1 checkpoints and their Git/test evidence; it does not replay transitions or create another live tracker. P1-05 is admitted from the approved dependency graph by the controller, then the tested scheduler governs later reservations; P1-06 enables normal result import. This one-time procedure exists only for this rebuild and is not a v1 migration or a future-run code path.
...
- `pipeline-run/v2` schema marker, run ID, base commit, target branch, worker limit, artifact paths, monotonic revision, and last transition identity;
...
The scheduler validates dependencies, task state, approved decisions, planned batch ordering, typed file/tree scope, active workers, and detected capacity. Readiness is advisory: actual reservations/starts are serialized under the tracker lock and revalidate questions, dependencies, pairwise candidate scopes, current ownership, and capacity. Several individually ready candidates cannot all reserve if they conflict with one another or a prior reservation consumes capacity. Compatible dependent tasks may stay with one executor in order. Independent, disjoint batches may run concurrently. Capacity is a ceiling, never a reason to dispatch unnecessary work or early/duplicate review.
...
Both gate types use `Critical`, `Important`, and `Minor`. Verified Critical/Important findings block. A missing approved requirement cannot be Minor. Every Minor is Fixed, Deferred with authority/impact, or Rejected with evidence. Gate closure is derived from evidence, never a caller-supplied boolean: required reports must name the correct gate and reviewed base/HEAD, required verification must cover that code state, no confirmed blocker or unresolved acceptance question may remain, every Minor needs a disposition, and every repository-changing fix needs its applicable re-review. A final-only phase cannot open a phase-review gate; the master gate separately enforces its two reviewer assignments.
