...
`F-004` and `F-005` are closed without repository changes, one as a user-ruled
false positive and one as `withdrawn` with its reason; neither names a commit
hash — the shapes the M=0 arm must accept. Put a hash in the withdrawn row's
...
| F-004 | Major | 2 | `src/c.py:5` | a fixture non-finding | false-positive | user-ruled false positive |
| F-005 | Major | 2 | `src/d.py:7` | a malformed fixture finding | withdrawn | withdrawn → malformed |
...
| 2 | Phase 2 | 1 | F-004, F-005 | F-002 | 2026-09-07 10:30 |
